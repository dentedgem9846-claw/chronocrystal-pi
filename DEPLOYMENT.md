...
- The entrypoint seeds `/app/state/data` from the image's `/app/data-seed` on first start, then runs the Bun app with cwd `/app/state/data` so Pi auto-discovers `.pi/SYSTEM.md` there.
